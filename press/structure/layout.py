...
class Layout(object):
    ...
    def generate_fstab(self, method='UUID'):
        ...
        header = '# Generated by Press v' + helpers.package.get_press_version()
        fstab = ''
        ...
        for disk in self.allocated:
            ...
            for partition in partition_table.partitions:
                ...
                uuid = self.udev.get_device_by_name(partition.devname).get('ID_FS_UUID')
                if not uuid:
                    continue

                label = self.udev.get_device_by_name(partition.devname).get('ID_FS_LABEL')
                if method == 'LABEL':
                    log.debug(
