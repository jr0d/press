...
def lookup_interface(interface, dev_missing_ok=False):
    ...
    if not lookup_method:
        raise NetworkConfigurationError('Press 0.3 Network configuration error, missing type')
    ...
    if not ndi:
        if ref['type'] == 'dev' and dev_missing_ok:
            ...
            raise NetworkConfigurationError('Press 0.3 Could not find device: %s' % ref['value'])
